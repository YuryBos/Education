...
function getRandomInt() {
    const min1 = 117;
    const max1 = 132;
    const out = Math.floor(min1 + Math.random() * (max1 + 1 - min1))
    out1.innerHTML = out;
}
getRandomInt();
...
// Чуть усложним. Сейчас первая задача жестко завязана на значения 117 и . Напишите функцию
// getRandomInt2, которая на основе min, max генерирует случайное целое число в этом диапазоне
//  и выводит в out-2. Запустите эту функцию. 
...
function getRandomInt2() {
    let out = Math.floor(min + Math.random() * (max + 1 - min))
    out2.innerHTML = out;
}
getRandomInt2();

// Task 3.
// Не забываем, что функции можно вызывать внутри другой функции. Напишите функцию t3, которая 
// срабатывает при клике по кнопке .b-3, и запускает фукнции hello, и f2021. Если все сделано 
// верно, то внутри .out-3 вы увидите текст hello 2021.
...
function odd() {
    out5.textContent = 'чет';
}

function even() {
    out5.textContent = 'нечет';
}

function t5() {
    const cont = Number(document.querySelector('.i-5').value);
    if (cont % 2 === 0) {
        odd();
    } else {
        even();
